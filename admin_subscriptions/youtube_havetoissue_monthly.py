...
class YoutubeHavetoissueMonthly(Screen):
    ...
    def delete_subscription(self, application_id):
        ...
        try:
            ...
            cursor.execute("DELETE FROM youtube_subscriptions WHERE application_id = ?", (application_id,))
            conn.commit()
    ...
    def issue_team(self, team_subscriptions):
        ...
        try:
            ...
            cursor.execute("SELECT MAX(team_id) FROM youtube_subscriptions")
    ...
    def go_back(self):
        self.manager.current = "admin_youtube_screen"  # Navigate back to the admin dashboard


# Load the kv file for UI definition
Builder.load_file("kv/youtube_havetoissue_monthly.kv")
