...
class NetflixHavetoissueYearly(Screen):
    subscriptions_container = ObjectProperty(None)  # Ensure default value is None
    ...
    def load_subscriptions(self):
        """Load all yearly subscriptions that are not yet issued."""
        try:
            ...
            cursor.execute(""" 
                ...
                WHERE plan = 'Yearly Plan' AND issued = 'no'""")
            rows = cursor.fetchall()

            if not self.subscriptions_container:
                print("subscriptions_container is None. Check the .kv file.")
                return
            ...
            self.show_popup("Error", f"Failed to load subscriptions: {str(e)}")
        finally:
            conn.close()
    ...
    def delete_subscription(self, application_id):
        ...
        try:
            ...
            cursor.execute("DELETE FROM netflix_subscriptions WHERE application_id = ?", (application_id,))
            conn.commit()
    ...
    def issue_team(self, team_subscriptions):
        ...
        try:
            ...
            cursor.execute("SELECT MAX(team_id) FROM netflix_subscriptions")
    ...
    def go_back(self):
        self.manager.current = "admin_netflix_screen"  # Navigate back to the admin dashboard

# Load the kv file for UI definition
Builder.load_file("kv/netflix_havetoissue_yearly.kv")