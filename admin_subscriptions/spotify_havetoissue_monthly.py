...
class SpotifyHavetoissueMonthly(Screen):
    ...
    def delete_subscription(self, application_id):
        ...
        try:
            ...
            cursor.execute("DELETE FROM spotify_subscriptions WHERE application_id = ?", (application_id,))
            conn.commit()
    ...
    def issue_team(self, team_subscriptions):
        ...
        try:
            ...
            cursor.execute("SELECT MAX(team_id) FROM spotify_subscriptions")
    ...
    def go_back(self):
        self.manager.current = "admin_spotify_screen"  # Navigate back to the admin dashboard


# Load the kv file for UI definition
Builder.load_file("kv/spotify_havetoissue_monthly.kv")
